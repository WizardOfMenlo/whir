    ...
    PowStrategy: nimue_pow::PowStrategy,
{
            ...
            + DigestReader<MerkleConfig>,
    {
        ...
        if self.params.initial_statement {
            ...
            // Initial sumcheck
            sumcheck_rounds
                .reserve_exact(self.params.folding_factor.get_folding_factor_of_round(0));
            for _ in 0..self.params.folding_factor.get_folding_factor_of_round(0) {
            ...
            let mut folding_randomness_vec =
                vec![F::ZERO; self.params.folding_factor.get_folding_factor_of_round(0)];
        ...
        let mut exp_domain_gen =
            domain_gen.pow([1 << self.params.folding_factor.get_folding_factor_of_round(0)]);
        ...
        for r in 0..self.params.n_rounds() {
            ...
            let stir_challenges_indexes = get_challenge_stir_queries(
                ...
                self.params
                    .folding_factor
                    .get_folding_factor_of_round(r + 1), // TODO: double check
            ...
            let mut sumcheck_rounds = Vec::with_capacity(
                self.params
                    .folding_factor
                    .get_folding_factor_of_round(r + 1),
            );
            for _ in 0..self
                .params
                .folding_factor
        ...
        let final_randomness_indexes = get_challenge_stir_queries(
            ...
            self.params
                .folding_factor
                .get_folding_factor_of_round(self.params.n_rounds()),
    ...
    fn compute_folds_full(&self, parsed: &ParsedProof<F>) -> Vec<Vec<F>> {
        ...
        let coset_domain_size = 1 << self.params.folding_factor.get_folding_factor_of_round(0);
            ...
            let evaluations: Vec<_> = round
                ...
                .map(|(index, answers)| {
                    ...
                    compute_fold(
                        ...
                        self.params
                            .folding_factor
                            .get_folding_factor_of_round(round_index + 1),
                    )
                })
                .collect();
        ...
        let evaluations: Vec<_> = parsed
            ...
            .map(|(index, answers)| {
                ...
                compute_fold(
                    ...
                    self.params
                        .folding_factor
                        .get_folding_factor_of_round(parsed.rounds.len()),
                )
            })
            .collect();
        result.push(evaluations);
