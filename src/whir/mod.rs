...
/// The proof generated by the WHIR prover and verified by the verifier.
...
{
    /// A list of Merkle proofs, one per round (including the final round), showing
    /// that the prover committed to the correct evaluations at queried positions.
    /// Each entry contains:
    /// - a Merkle `MultiPath` for a batch proof
    /// - a vector of leaf openings used to evaluate STIR queries and final checks.
    #[serde(with = "crate::ark_serde")]
    pub merkle_paths: Vec<(MultiPath<MerkleConfig>, Vec<Vec<F>>)>,

    /// Contains the evaluations of all linear constraints in the final `Statement`
    /// at the verifier's final randomness point. These values are needed by the
    /// verifier to check consistency of the final folded statement with the sumcheck.
    #[serde(with = "crate::ark_serde")]
