...
/// Blinding evaluations at one round-0 query point `gamma`.
///
/// Values are represented in the doc-faithful beq basis at `(gamma, -rho)`:
/// - `m_eval = M(gamma, -rho)`,
...
impl<F: FftField> BlindingEvaluations<F> {
    /// Compute `h(gamma) = m(gamma,-rho) + Σ_j beta^j * gamma^(2^(j-1)) * g_hat_j(gamma,-rho)`.
...
/// - `msk(z)` and `g0_hat(z)` on `ell` variables,
...
pub struct BlindingPolynomials<F: FftField> {
    pub msk: Vec<F::BasePrimeField>,
    pub g0_hat: Vec<F::BasePrimeField>,
...
impl<F: FftField> BlindingPolynomials<F> {
    pub fn sample<R: RngCore + CryptoRng>(
        ...
        // Keep `poly_idx` in signature for stable per-polynomial call shape.
        let _ = poly_idx;
    ...
    pub fn evaluate_at(
        ...
        debug_assert_eq!(self.m_poly.len(), self.msk.len() * 2);
        debug_assert!(self.g_hats.iter().all(|g_hat| g_hat.len() == self.msk.len()));
        let num_blinding_variables = self.msk.len().ilog2() as usize;
        let beq_weights =
            eq_weights_at_gamma(gamma, masking_challenge, num_blinding_variables, self.m_poly.len());
...
pub fn compute_per_polynomial_claims<F: FftField>(
    ...
    // Batch query claims with tau2: Σ_i tau2^i * eval_i.
...
fn eq_weights_at_gamma<F: FftField>(
    ...
    debug_assert_eq!(output_size, z_eq.len() * 2);
    let eq_neg_masking_at_0 = F::ONE + masking_challenge;
    let eq_neg_masking_at_1 = -masking_challenge;
    let mut beq_weights = vec![F::ZERO; output_size];
    for (z_idx, &beq_z) in z_eq.iter().enumerate() {
        beq_weights[2 * z_idx] = eq_neg_masking_at_0 * beq_z;
        beq_weights[2 * z_idx + 1] = eq_neg_masking_at_1 * beq_z;
    }
    beq_weights
}
...
mod tests {
    ...
    fn test_evaluate_at_matches_beq_layout_openings() {
        ...
        let g_hats = (0..num_witness_variables)
            ...
            .collect::<Vec<_>>();
        let polys = BlindingPolynomials::<EF> {
            msk,
            g0_hat,
            m_poly,
            g_hats,
        ...
        assert_eq!(eval.m_eval, covector.evaluate(&embedding, &layout_vectors[0]));
        for (g_idx, g_eval) in eval.g_hat_evals.iter().enumerate() {
            assert_eq!(*g_eval, covector.evaluate(&embedding, &layout_vectors[1 + g_idx]));
        }
    }
