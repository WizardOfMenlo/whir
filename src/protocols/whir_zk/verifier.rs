...
impl<F: FftField> Config<F> {
    ...
    /// plus a single batch commitment for all helper polynomials.
    ///
    /// Returns `(f_hat_commitments, helper_commitment)`.
        ...
        Hash: ProverMessage<[H::U]>,
    {
        ...
        let helper_commitment = self.helper.receive_commitment(verifier_state)?;
    ...
    /// Open f̂ commitments and verify helper evaluations, returning lifted
    ...
    fn open_and_verify_initial_helpers<H>(
        &self,
        verifier_state: &mut VerifierState<'_, H>,
        f_hat_commitments: &[&Commitment<F>],
        helper_commitment: &Commitment<F>,
        ...
        Hash: ProverMessage<[H::U]>,
    {
        ...
        let virtual_oracle_values = self.verify_helper_evaluations(
            verifier_state,
            &in_domain_base,
            helper_commitment,
            challenges,
    ...
    where
        ...
        Hash: ProverMessage<[H::U]>,
    {
        let (in_domain, virtual_oracle_values) = self.open_and_verify_initial_helpers(
            verifier_state,
            f_hat_commitments,
            helper_commitment,
            challenges,
    ...
    /// Verify the initial commitment opening: verify [[f̂]], read + verify helper
    ...
    fn verify_helper_evaluations<H>(
        &self,
        verifier_state: &mut VerifierState<'_, H>,
        ...
        Hash: ProverMessage<[H::U]>,
    {
        ...
        let num_witness_vars = self.zk_params.num_witness_variables;
        ...
        for &alpha_base in &in_domain_base.points {
            ...
            for gamma in coset_gammas {
                for poly_idx in 0..num_polys {
                    ...
                    helper_evals_per_poly[poly_idx].push(HelperEvaluations {
                        gamma,
                        m_eval,
                        g_hat_evals,
                    });
        ...
        let beq_weights = construct_batched_eq_weights(
            ...
            self.zk_params.num_helper_variables,
        );
                ...
                let combined_coset_values: Vec<F> = coset_gammas
                    ...
                    .map(|(coset_elem_idx, &gamma_ext)| {
                        ...
                        for poly_idx in 0..num_polys {
                            ...
                            let helper_value = helper_evals_per_poly[poly_idx]
