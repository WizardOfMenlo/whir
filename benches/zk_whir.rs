...
fn make_polynomials(num_variables: usize, num_polynomials: usize) -> Vec<Vec<F>> {
...
fn zk_v1_commit(bencher: Bencher, num_variables: usize) {
    ...
    // Flatten: [f̂₁, g₁, f̂₂, g₂]
    let commit_polys: Vec<&[F]> = bundles
        .iter()
        .flat_map(|bundle| [bundle.f_hat.as_slice(), bundle.g_poly.as_slice()])
        .collect();
...
fn zk_v1_prove(bencher: Bencher, num_variables: usize) {
    ...
    let p_refs: Vec<&[F]> = p_polys.iter().map(Vec::as_slice).collect();
    ...
    bencher
        ...
        .bench_values(|(mut prover_state, witness)| {
            let prove_forms: Vec<Box<dyn LinearForm<F>>> = vec![Box::new(
                MultilinearExtension::new(weights[0].point.clone()),
            )];
            let _ = black_box(prove_config.prove(
                &mut prover_state,
...
fn zk_v1_verify(bencher: Bencher, num_variables: usize) {
    ...
    let proof = {
        let mut prover_state = ProverState::new_std(&ds);
        let witness = prove_config.commit(&mut prover_state, &p_refs);
        let prove_forms: Vec<Box<dyn LinearForm<F>>> = vec![Box::new(MultilinearExtension::new(
            weights[0].point.clone(),
        ))];
        let _ = prove_config.prove(
            &mut prover_state,
            p_refs.iter().map(|v| Cow::Borrowed(*v)).collect(),
            vec![Cow::Borrowed(&witness)],
            prove_forms,
